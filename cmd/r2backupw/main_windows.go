...
// Command r2backupw runs r2backup with no console window.
//
...
// of its code runs, so r2backup cannot win this from the inside: it tries
...
// allocates one, and it starts r2backup with CREATE_NO_WINDOW so the child
...
// `r2backup update`, and a missed update would leave the scheduler quietly
// running last month's backup logic. This has no version and no logic to go
// stale: it runs whatever r2backup.exe is sitting next to it, so an update
// that only replaces r2backup.exe is still completely correct.
package main
...
func run() int {
	...
	target := filepath.Join(filepath.Dir(exe), "r2backup.exe")
	if _, err := os.Stat(target); err != nil {
		return fail(fmt.Sprintf("find r2backup.exe beside %s", exe), err)
	}
...
func fail(what string, err error) int {
	msg := fmt.Sprintf("%s r2backupw: could not %s: %v\n",
